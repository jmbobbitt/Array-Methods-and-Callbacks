...
function filterToObj(array, year) {
    let objIWantInArray = array.filter (e=> e["Year"] === year);
     //filter obj by year
     let objIWant = {};
     objIWant = objIWantInArray[0]; 
...
function winnerOfWorldCup(array, year) {
    let obj = filterToObj(array, year); 
    // get target object in array by year.  Callback function.
...
function getFinals(data) {
    ...
    let newArray = data.map(element => {
        let newObj = {}; 
        //declare newObj, assign target properties below
...
//getFinals return smaller objects in an array

let newArrayOfYears = finalsArray.map (element => {
    //creates an array of years in integers
        let year = 0; 
...
function getWinners(data, callback) {
   ...
   let finalsArray = getFinals(data); 
   //returns an array of smaller objects with only finals data
...
getWinnersByYear()     
    ...
    let winnerStatement = years.map((year, i) => { 
        // declare var for index
    ...
    return winnerStatement.join(" "); 
    // convert array to a set of strings seperated by a space
...
function getHomeTeamGoals(partial, next) {
     // create helper func to get total home goals
...
function getAwayTeamGoals(partial, next) { 
    // create helper func to get total away goals
    let entry = next;
...
let homeTotal = data.reduce(getHomeTeamGoals, 0); 
// gives total home goals
let awayTotal = data.reduce(getAwayTeamGoals, 0); 
// gives total home goals

aveHome = homeTotal/data.length; 
// calculate home average
aveHome = aveHome.toFixed(2);  
// fix decimals to 2

aveAway = awayTotal/data.length; 
// calculate away average
aveAway = aveAway.toFixed(2);  
// fix decimals to 2
